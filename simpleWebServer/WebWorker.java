...
private void readHTTPRequest(InputStream is)
{
   ...
   while (true) {
      try {
         ...
         line = r.readLine();
         if(line.contains("GET")){						// Finds GET request and strips it of the requested file path
            path = line.substring(5, line.length()-1);
            path = path.substring(0,path.indexOf(' '));
			file = new File(path);
            // System.err.println(file.exists());	Debug statement
            }
...
//OutputStreams only take binary and the file doesn't have a getByte function like String so I have to create my own.
{
    ...
    fis.read(bStream);
    fis.close();
	//String s = new String(bStream);				//FIXME getting a weird encoding on print statement
    //System.out.println(s);
	//s.replace("<cs371date>", ("<h2> " + date.toString() + " </h2>"));
	//s.replace("<cs371server>", "<h2>Welcome to my server!</h2>");
	//bStream = s.getBytes();
    return bStream;
}
} // end class
